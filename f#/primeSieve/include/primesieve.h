 ...
 */
PRIMESIEVE_EXPORT void* primesieve_generate_primes(uint64_t start, uint64_t stop, size_t* size, int type);

/** Get an array with the first n primes >= start.
 *  @param type  The type of the primes to generate, e.g. INT_PRIMES.
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT void* primesieve_generate_n_primes(uint64_t n, uint64_t start, int type);
 ...
 */
PRIMESIEVE_EXPORT uint64_t primesieve_nth_prime(int64_t n, uint64_t start);
 ...
 *  @pre   start <= 2^64 - 2^32 * 11.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_parallel_nth_prime(int64_t n, uint64_t start);

/** Count the primes within the interval [start, stop].
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_count_primes(uint64_t start, uint64_t stop);

/** Count the twin primes within the interval [start, stop].
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_count_twins(uint64_t start, uint64_t stop);

/** Count the prime triplets within the interval [start, stop].
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_count_triplets(uint64_t start, uint64_t stop);

/** Count the prime quadruplets within the interval [start, stop].
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_count_quadruplets(uint64_t start, uint64_t stop);

/** Count the prime quintuplets within the interval [start, stop].
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_count_quintuplets(uint64_t start, uint64_t stop);

/** Count the prime sextuplets within the interval [start, stop].
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_count_sextuplets(uint64_t start, uint64_t stop);
 ...
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_parallel_count_primes(uint64_t start, uint64_t stop);
 ...
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_parallel_count_twins(uint64_t start, uint64_t stop);
 ...
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_parallel_count_triplets(uint64_t start, uint64_t stop);
 ...
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_parallel_count_quadruplets(uint64_t start, uint64_t stop);
 ...
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_parallel_count_quintuplets(uint64_t start, uint64_t stop);
 ...
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_parallel_count_sextuplets(uint64_t start, uint64_t stop);

/** Print the primes within the interval [start, stop]
 *  to the standard output.
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT void primesieve_print_primes(uint64_t start, uint64_t stop);

/** Print the twin primes within the interval [start, stop]
 *  to the standard output.
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT void primesieve_print_twins(uint64_t start, uint64_t stop);

/** Print the prime triplets within the interval [start, stop]
 *  to the standard output.
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT void primesieve_print_triplets(uint64_t start, uint64_t stop);

/** Print the prime quadruplets within the interval [start, stop]
 *  to the standard output.
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT void primesieve_print_quadruplets(uint64_t start, uint64_t stop);

/** Print the prime quintuplets within the interval [start, stop]
 *  to the standard output.
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT void primesieve_print_quintuplets(uint64_t start, uint64_t stop);

/** Print the prime sextuplets within the interval [start, stop]
 *  to the standard output.
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT void primesieve_print_sextuplets(uint64_t start, uint64_t stop);

/** Call back the primes within the interval [start, stop].
 *  @param callback  A callback function.
 *  @pre stop <= 2^64 - 2^32 * 10.
 */
PRIMESIEVE_EXPORT void primesieve_callback_primes(uint64_t start, uint64_t stop, void (*callback)(uint64_t prime));

/** Get the current set sieve size in kilobytes. */
PRIMESIEVE_EXPORT int primesieve_get_sieve_size();

/** Get the current set number of threads.
 *  @note By default MAX_THREADS (-1) is returned.
 */
PRIMESIEVE_EXPORT int primesieve_get_num_threads();

/** Returns the largest valid stop number for primesieve.
 *  @return (2^64-1) - (2^32-1) * 10.
 */
PRIMESIEVE_EXPORT uint64_t primesieve_get_max_stop();
 ...
 */
PRIMESIEVE_EXPORT void primesieve_set_sieve_size(int sieve_size);
 ...
 */
PRIMESIEVE_EXPORT void primesieve_set_num_threads(int num_threads);

/** Deallocate a primes array created using the
 *  primesieve_generate_primes() or primesieve_generate_n_primes()
 *  functions.
 */
PRIMESIEVE_EXPORT void primesieve_free(void* primes);
 ...
 */
PRIMESIEVE_EXPORT int primesieve_test();

/** Get the primesieve version number, in the form “i.j.k”. */
PRIMESIEVE_EXPORT const char* primesieve_version();
